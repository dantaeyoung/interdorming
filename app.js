...
async function autoLoadDefaultRooms() {
    try {
        ...
        const csvText = await response.text();
        parseMonasteryRoomConfig(csvText);
...
// Parse the monastery's specific CSV format (two-column layout with guest data)
function parseMonasteryRoomConfig(csvText) {
    const lines = csvText.trim().split('\n');
    const dormitories = [];

    // Track current state for both columns
    let currentDorm1 = null;
    let currentRoom1 = null;
    let currentGender1 = null;

    let currentDorm2 = null;
    let currentRoom2 = null;
    let currentGender2 = null;

    // Process each line (skip header row)
    for (let i = 1; i < lines.length; i++) {
        const parts = lines[i].split(',');

        // Left column (columns 0-9)
        const dorm1 = parts[1]?.trim();
        const room1 = parts[2]?.trim();
        const bed1 = parts[3]?.trim();
        const bedType1 = parts[4]?.trim();
        const gender1 = parts[8]?.trim(); // Gender marker

        // Right column (columns 11-19)
        const dorm2 = parts[11]?.trim();
        const room2 = parts[12]?.trim();
        const bed2 = parts[13]?.trim();
        const bedType2 = parts[14]?.trim();
        const gender2 = parts[18]?.trim(); // Gender marker

        // Process left column
        if (dorm1 && dorm1 !== 'F' && dorm1 !== 'M' && dorm1 !== 'W / family' && dorm1 !== 'Family/Couple') {
            currentDorm1 = dorm1;
            currentGender1 = null;
        }
        if (gender1 === 'F' || gender1 === 'M') {
            currentGender1 = gender1;
        }
        if (room1 && currentDorm1) {
            currentRoom1 = room1;
        }
        if (bed1 && currentDorm1 && currentRoom1) {
            addBedToConfig(dormitories, currentDorm1, currentRoom1, bed1, bedType1, currentGender1);
        }

        // Process right column
        if (dorm2 && dorm2 !== 'F' && dorm2 !== 'M' && dorm2 !== 'W / family' && dorm2 !== 'Family/Couple') {
            currentDorm2 = dorm2;
            currentGender2 = null;
        }
        if (gender2 === 'F' || gender2 === 'M') {
            currentGender2 = gender2;
        }
        if (room2 && currentDorm2) {
            currentRoom2 = room2;
        }
        if (bed2 && currentDorm2 && currentRoom2) {
            addBedToConfig(dormitories, currentDorm2, currentRoom2, bed2, bedType2, currentGender2);
        }
    }

    // Assign colors to dormitories
    const colors = ['#3b82f6', '#ef4444', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899', '#06b6d4', '#84cc16'];
    dormitories.forEach((dorm, i) => {
        dorm.color = colors[i % colors.length];
    });

    // Update app state
    app.dormitories = dormitories;
    app.refreshRoomsFromDormitories();
    app.saveToLocalStorage();

    console.log('Parsed monastery room config:', dormitories);
}

function addBedToConfig(dormitories, dormName, roomNum, bedId, bedTypeRaw, gender) {
    // Find or create dormitory
    let dorm = dormitories.find(d => d.name === dormName);
    if (!dorm) {
        dorm = {
            id: `dorm_${dormitories.length + 1}`,
            name: dormName,
            color: '#3b82f6',
            active: true,
            rooms: []
    ...
    // Find or create room
    let room = dorm.rooms.find(r => r.roomName === roomNum);
    if (!room) {
        room = {
            id: `${dorm.id}_room_${dorm.rooms.length + 1}`,
            roomName: roomNum,
            roomGender: gender === 'F' ? 'Female' : gender === 'M' ? 'Male' : 'Coed',
            beds: []
        };
        dorm.rooms.push(room);
    }

    // Parse bed type from format like "1L", "2L", "3/Single", "4/Single", "5"
    let bedType = 'single';
    if (bedId.includes('L') || bedTypeRaw?.toLowerCase().includes('lb')) {
        bedType = 'lower';
    } else if (bedId.includes('U')) {
        bedType = 'upper';
    } else if (bedId.includes('Single') || bedId.includes('/')) {
        bedType = 'single';
    }

    // Generate bed ID
    const bedIdClean = bedId.replace(/[^0-9]/g, '');
    const generatedBedId = `${dormName.substring(0, 2).toUpperCase()}${roomNum}${bedIdClean}`;

    // Add bed if not already present
    const bedExists = room.beds.some(b => b.bedId === generatedBedId);
    if (!bedExists) {
        room.beds.push({
            bedId: generatedBedId,
            bedType: bedType,
            position: room.beds.length + 1,
            assignedGuestId: null
        });
    }
}